...
async function waitConnection() {
    mongoose.connection.readyState == 1
        ...
        ? await new Promise((resolve) =>
              mongoose.connection.on('connected', () => resolve(undefined))
          )
...
export async function create(entity, { ...data }) {
    if (!entity || typeof entity !== 'string')
        throw new Error('Имя сущности должно быть строкой!')
...
export async function readOne(entity, params) {
    ...
    try {
        ...
        const instance = await Model.findOne(params).exec()
        mongoose.disconnect()
        return instance
    } catch (e) {
        console.error(e)
...
export async function read(entity, params) {
    ...
    try {
        ...
        const instances = await Model.find(params).exec()
        mongoose.disconnect()
        return instances
    } catch (e) {
        console.error(e)
...
async function checks(entity, params, cb) {
    if (typeof entity !== 'string')
        throw new Error('Имя сущности должно быть строкой!')
    await waitConnection()
    try {
